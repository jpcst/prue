...
const IP_PATH: &'static str = "C:/Users/hollics/Documents/ip.txt";
const KEY_PATH: &'static str = "C:/Users/hollics/Documents/key.txt";
...
pub fn is_on(list: Vec<u8>) -> Vec<bool> { // returns a vector with light states (on->true/off->false)
    ...
    for i in 0..size {
        ...
        let js = serde_json::to_string(&data[format!("{}", i+1)]["state"]["on"]).unwrap();
...
pub fn brightness(list: Vec<u8>) -> Vec<u8> { // returns a vector with light states (on->true/off->false)
    ...
    for i in 0..size {
        if list[i] == 1 {
            let js = serde_json::to_string(&data[format!("{}", i+1)]["state"]["bri"]).unwrap();
...
pub fn names(list: Vec<u8>) -> Vec<String> {
    ...
    for i in 0..size {
        let js = serde_json::to_string(&data[format!("{}", i+1)]["name"]).unwrap();
